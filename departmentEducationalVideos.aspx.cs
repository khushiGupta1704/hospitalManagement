...
public partial class departmentEducationalVideos : System.Web.UI.Page
{
    ...
    private void show_data()
    {
        ...
        dt = verma.Show_Data("Select * from departments where id='" + Page.RouteData.Values["id"] + "'");
        if (dt.Rows.Count > 0)
        {
            lable_title.Text = dt.Rows[0]["title"].ToString();
            DataTable dt1 = new DataTable();
            dt1 = verma.Show_Data("select * from educational_videos where status=1 and department_id= '"+ Page.RouteData.Values["id"] + "'order by position");
...
//        DataTable dt = new DataTable();
//        Repeater Repeater4 = ((Repeater)e.Item.FindControl("TeamRepeater"));
...
//        string query = "SELECT our_team.* FROM our_team INNER JOIN departments ON our_team.department_id = departments.id where our_team.department_id='" + id + "' and our_team.status=1 order by our_team.position";
