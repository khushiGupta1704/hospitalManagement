...
public partial class blogDetail : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            DepartmentCountRepeater.DataSource = verma.Show_Data("select count(*) as blog_count,departments.title as exp1 from blogs INNER JOIN departments ON blogs.department_id = departments.id where blogs.status=1 group by departments.title");
            DepartmentCountRepeater.DataBind();
            BlogRepeater.DataSource = verma.Show_Data("SELECT TOP 5 * from blogs where status=1 order by position");
    ...
    private void show_data()
    {
        ...
        if (dt.Rows.Count>0)
        {
            ...
            detail_image.Attributes.Add("style", "background-image: url('http://aastha.omtechnoware.com/" + dt.Rows[0]["image"].ToString() + "')");
    ...
    protected void Comment_message_Click(object sender, EventArgs e)
    {
        ...
        //else
        //{
        //    Response.Redirect("http://aastha.omtechnoware.com/Login");
